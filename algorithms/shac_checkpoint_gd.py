    ...
    def __init__(self, cfg):
        ...
        if cfg['params']['general']['train']:
            ...
            if 'general' in save_cfg['params']:
                deleted_keys = [key for key in save_cfg['params']['general'].keys() if key in save_cfg['params']['config']]
        ...
        actor_fn = getattr(models.actor_checkpoint_gd, self.actor_name)
        self.actor = actor_fn(self.num_obs, self.num_actions, cfg['params']['network'], device=self.device)
        critic_fn = getattr(models.critic_checkpoint_gd, self.critic_name)
    ...
    def compute_actor_loss(self, deterministic=False):
        ...
        for i in range(self.steps_num):
            ...
            for id in done_env_ids:
                if torch.isnan(extra_info['obs_before_reset'][id]).sum() > 0 or torch.isinf(extra_info['obs_before_reset'][id]).sum() > 0 or (torch.abs(extra_info['obs_before_reset'][id]) > 1e6).sum() > 0:
